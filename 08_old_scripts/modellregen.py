...
sys.path.insert(0, os.path.join(current_path, '02_input_data')) #working directory needs to be \urbanml


import matplotlib.pyplot as plt
import pandas as pd
import modules.get_kostra as get_kostra
from datetime import timedelta

# path to the kostra raw data
Kostra_raw_path = os.path.join(current_path, '02_input_data\\Kostra')
...
for j in jaerlichkeiten:
    for d in dauern:
        ...
        euler2[j].to_csv(f'02_input_data\\modellregen\\euler2_T{j}D{int(d)}.csv', sep=',', header=True, index=True)
