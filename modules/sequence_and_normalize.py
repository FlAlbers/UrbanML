...
def sequence_list(sims_data, in_vars=['duration', 'p'], out_vars=None, in_scaler=None, out_scaler=None, lag = 36, delay = 0, prediction_steps = 12, buffer_time = pd.Timedelta('2h')):
    ...
    for sample in sims_data:
        ...
        in_seq, out_seq = sequence_data([sample], in_vars=in_vars, out_vars=out_vars, in_scaler=None, out_scaler=None, lag=lag, delay=delay, prediction_steps=prediction_steps)
        in_seq_trans, out_seq_trans = sequence_data([sample], in_vars=in_vars, out_vars=out_vars, in_scaler=in_scaler, out_scaler=out_scaler, lag=lag, delay=delay, prediction_steps=prediction_steps)
